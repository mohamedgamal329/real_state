...
class ArchivePropertiesBloc
  ...
  Future<void> _onStarted(
    ...
    await _runGuardedRequest(
      ...
      job: () async {
        ...
        await _loadPage(emit, filter: filter);
      },
    );
  }
  ...
  Future<void> _onRefreshed(
    ...
    final filter = event.filter ?? current?.filter ?? const PropertyFilter();
    await _runGuardedRequest(
      isRefresh: true,
      job: () async {
        if (current == null) {
        ...
        await _loadPage(emit, previous: current, filter: filter);
      },
    );
  }
  ...
  Future<void> _onRetry(
    ...
    final current = _asLoaded(state);
    if (current != null) {
      add(ArchivePropertiesRefreshed(filter: current.filter));
    } else {
  ...
  Future<void> _onExternalMutation(
    ...
    await _runGuardedRequest(
      ...
      job: () async {
        ...
        emit(ArchivePropertiesActionInProgress(current));
        await _loadPage(emit, previous: current, filter: current.filter);
      },
    );
  }
  ...
  Future<void> _loadPage(
    ...
    try {
      ...
      final fetchedAreas = await _fetchAreaNamesFor(page.items);
      final areaNames = previous != null
          ? {...previous.areaNames, ...fetchedAreas}
          : fetchedAreas;
      if (previous != null) {
  ...
  Future<Map<String, LocationArea>> _fetchAreaNamesFor(
    ...
    final known = _asLoaded(state)?.areaNames.keys.toSet() ?? <String>{};
    final ids = props
