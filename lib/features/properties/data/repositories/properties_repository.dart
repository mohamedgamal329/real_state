...
class PageResult<T> {
  ...
  PageResult({required this.items, this.lastDocument, required this.hasMore});
}
...
class PropertiesRepository {
  ...
  PropertiesRepository(this._firestore, {String? collection})
  ...
  Future<Set<String>> fetchBrokerAreaIds(String brokerId, {UserRole? role}) async {
    if (role == UserRole.collector) {
      throw const LocalizedException('access_denied_broker_data');
    }
    try {
      ...
      throw LocalizedException('load_failed', args: [mapExceptionToFailure(e).toString()]);
    }
  }
  ...
  Future<PageResult<Property>> _fetchScopedPage({
    ...
    final hasPriceInequality = (filter?.minPrice != null) || (filter?.maxPrice != null);
    ...
    try {
      while (results.length < limit && moreServer) {
        final snap = await _applyPagination(baseQuery, limit: chunk, startAfter: cursor).get();
  ...
  Future<PageResult<Property>> fetchPage({
    ...
    final hasPriceInequality = (filter?.minPrice != null) || (filter?.maxPrice != null);
    final baseQuery = hasPriceInequality ? _buildPriceQuery(filter) : _buildBaseQuery(filter);
    final chunk = limit * 3; // fetch more per round to compensate for client-side filtering
    final List<Property> results = [];
    var cursor = startAfter;
    DocumentSnapshot<Map<String, dynamic>>? lastDoc;
    var moreServer = true;
  ...
  Query<Map<String, dynamic>> _collectionRoot() => _firestore.collection(_collection);

  /// Returns whether this filter requires ordering by price (i.e. an inequality)
  ...
  bool _matchesClientFilter(Property property, PropertyFilter? filter) {
    if (property.isDeleted || property.status != PropertyStatus.active) return false;
    if (filter?.locationAreaId != null && property.locationAreaId != filter!.locationAreaId) {
      return false;
    }
  ...
  Future<Property> updateProperty({
    ...
    if (!canModifyProperty(property: existing, userId: userId, role: userRole)) {
    ...
    if (ownerPhoneEncryptedOrHiddenStored != null) {
      updateMap['ownerPhoneEncryptedOrHiddenStored'] = ownerPhoneEncryptedOrHiddenStored;
    }
  ...
  Future<Property> _archiveWithPermission({
    ...
    if (existing != null &&
        !canArchiveOrDeleteProperty(property: existing, userId: userId, role: userRole)) {
  ...
  Future<void> deleteProperty({
    ...
    final existing = PropertyDto.fromDoc(doc);
    if (!canArchiveOrDeleteProperty(property: existing, userId: userId, role: userRole)) {
      throw const LocalizedException('access_denied_delete');
    }
    await ref.update({
      'isDeleted': true,
